...
def find_authors_nationalities():
    ...
    for author in authors:
        author_list.append(f"{author.first_name} {author.last_name} is {author.nationality}")
...
def filter_authors_by_nationalities(auth_nationality):
    ordered_authors = Author.objects.filter(nationality=auth_nationality).order_by('first_name', 'last_name')
...
def filter_authors_by_birth_year(start_year, end_year):
    ordered_authors = Author.objects.filter(
        birth_date__year__gte=start_year, birth_date__year__lte=end_year).order_by('-birth_date')
